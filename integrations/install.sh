#!/usr/bin/env bash
# DeepReview installer — installs the skill for every supported coding agent.
#
...
if want opencode; then
  hdr "OpenCode"
  dest="$HOME/.config/opencode/command/$NAME.md"
  mkdir -p "$(dirname "$dest")"
  sed "s|__SKILL_DIR__|$HOME/.agents/skills/$NAME|g" \
      "$HERE/opencode-command.md" > "$dest"
  ok "slash command -> $dest  (uses /deep-review; reads the skill from ~/.agents/skills/$NAME)"
...
if want workbuddy; then
  ...
  ok "Reference file: integrations/workbuddy-skill.yml.md"
fi
...
hdr "Done"
ok "verify: SKILL.md exists in the paths above; then just ask your agent for a 深度调研."
