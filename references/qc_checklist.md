...
* [ ] **DOCX table typography uniform across pages**: Arial (sans) at ONE size (9 pt) for every table while body stays serif — per-table font sizes read as inconsistent pages; header row centred, bold, min height 0.8 cm (`trHeight atLeast`) with 2/4 pt spacing; body cells: narrow label-like columns (≤ ~26 chars) centred, long prose columns left; no bold in body cells (header only)
